...
export const LearningInsight: React.FC<LearningInsightProps> = ({
  ...
  return (
    <div className="learning-insight w-full bg-white/90 rounded-[22px] border border-[#cfe1f4] p-3 sm:p-4 shadow-card-soft relative overflow-hidden">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-3 items-center">
        ...
        <div className="lg:col-span-6 space-y-2">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-full bg-amber-400/20 text-amber-600 flex items-center justify-center shrink-0">
              <Lightbulb className="w-5 h-5 fill-amber-400 stroke-[2]" />
            </div>
            <h3 className="text-lg sm:text-xl font-extrabold text-[#15213D] tracking-tight">
              {title}
            </h3>
          </div>

          <p className="text-sm text-[#536178] leading-relaxed font-medium">
            {explanation}
          </p>
        ...
        <div className="lg:col-span-6 grid sm:grid-cols-3 gap-2">
          {pillars.map((pillar, i) => (
            <div 
              key={i}
              className="bg-white rounded-2xl p-3 border border-[#BBDDFF]/60 shadow-2xs flex items-start gap-3 transition-transform hover:translate-x-1"
            >
              <div 
